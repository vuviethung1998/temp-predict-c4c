...
def model_layer(model_type, optimizer  , input_dim, output_dim, rnn_units, rnn_layers,drop_out=0):
    # Model
...
def get_data_by_date(day, month, year, type):
    data = pd.read_csv(data_dir + 'month_day_year_temp.csv')
...
def get_temp(day, month,year, type='day'):
    ...
    data = get_data_by_date(day, month, year, type)
    if data == -1:
        return "Previous date not found in data store!!!"

    # model_type, optimizer  , input_dim, output_dim, rnn_units, rnn_layers,drop_out=0
    loaded_model, encoder_model, decoder_model = model_layer(model_type=model_type, optimizer=optimizer ,\
...
if __name__=="__main__":
    predicted_data = get_temp(5, 6, 2017, 'month')
